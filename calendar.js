...
function drawCld(SY,SM) {
    ...
    for(i=0;i<42;i++) {
        ...
        if(sD>-1 && sD<cld.length) {
            ...
            if(cld[sD].isToday){
                //设置今天的背景色
                sObj.style.background = '#BCDEF7';
                lObj.style.background = '#BCDEF7';
            }
...
function mOvr(v) {
    ...
    if( sObj.innerHTML != ''&& d>=0 ) {
        ...
        datedetail.innerHTML = cld[d].sYear +' 年 '+ cld[d].sMonth
            ...
            + cMonth(cld[d].lMonth) + ' 月 ' + cDay(cld[d].lDay) + '</span>';
...
function initial() {
    ...
    d = d<10?"0"+ d:d;
    m = m<10?"0"+ m:m;　
    document.getElementById("history").href="http://hao.360.cn/histoday/"+ m + d + ".html?src=onebox";
}
